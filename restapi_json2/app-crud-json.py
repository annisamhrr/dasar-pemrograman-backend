...
def clients():
    ...
    clients_str = f.read()
    all_client = json.loads(clients_str)
...
def client(id):
    ...
    return render_template("client.htmml", client=client_by_id)
...
def client_edit(id):
    ...
    client_by_id = list(filter(lambda x: (x["id"] == int(id)), all_clients))
    client_by_id = client_by_id[0]
    client_by_id = all_clients.index(client_by_id)
