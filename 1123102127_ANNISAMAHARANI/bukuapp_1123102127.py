...
def update_buku(id):
    try:
        ...
        with db.cursor() as cursor:
            # Cek apakah buku ada
            cursor.execute("SELECT * FROM buku_1123102127 WHERE id=%s", (id,))
...
def index():
    with db.cursor() as cursor:
        cursor.execute("SELECT * FROM buku_1123102127")
        books = cursor.fetchall()
    return render_template('index_1123102127.html', books=books)
...
def tambah():
    if request.method == 'POST':
        ...
        if not judul or not penulis or not tahun:
            return render_template('tambah_1123102127.html', error="Judul, penulis, dan tahun harus diisi")

        with db.cursor() as cursor:
            cursor.execute("INSERT INTO buku_1123102127 (judul, penulis, tahun) VALUES (%s, %s, %s)", 
                         (judul, penulis, tahun))
        db.commit()
...
def edit(id):
    if request.method == 'POST':
        ...
        if not judul or not penulis or not tahun:
            return render_template('edit_1123102127.html', id=id, error="Judul, penulis, dan tahun harus diisi")

        with db.cursor() as cursor:
            cursor.execute("UPDATE buku_1123102127 SET judul=%s, penulis=%s, tahun=%s WHERE id=%s",
    ...
    with db.cursor() as cursor:
        cursor.execute("SELECT * FROM buku_1123102127 WHERE id=%s", (id,))
        book = cursor.fetchone()
    return render_template('edit_1123102127.html', book=book)
